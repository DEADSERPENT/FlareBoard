...
class AuthController {
  async register(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // All registrations get Member role — Admin is seeded only
      ...
      if (!memberRole) {
        memberRole = await prisma.role.upsert({
          ...
          create: {
            name: 'Member',
            permissions: { canManageUsers: false, canManageProjects: true, canManageSettings: false, canViewAnalytics: true },
          },
        })
        await prisma.role.upsert({
          ...
          create: {
            name: 'Admin',
            permissions: { canManageUsers: true, canManageProjects: true, canManageSettings: true, canViewAnalytics: true },
          },
        })
      }
      ...
      const token = jwt.sign(
        { userId: user.id, email: user.email, roleId: user.roleId, roleName: memberRole.name },
  ...
  async refreshToken(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      // @ts-expect-error - jwt types issue with expiresIn string
      const token = jwt.sign(
        { userId: req.user.userId, email: req.user.email, roleId: req.user.roleId, roleName: req.user.roleName },
        process.env.JWT_SECRET!,
        { expiresIn }
      )
